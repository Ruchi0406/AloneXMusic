    ...
    if len(data.split()) < 2:
        mm = [
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(f"💒 Usage:\n\n@VeenaMusic_bot [ USERNAME | ID ] [ TEXT ]"),
...
@app.on_callback_query(filters.regex(pattern=r"fdaywhisper_(\d+)_(\d+)_?(\w*)"))
async def whispes_cb(_, query):
    ...
    SWITCH = InlineKeyboardMarkup([[InlineKeyboardButton("Go Inline 🪝", switch_inline_query="")]])
